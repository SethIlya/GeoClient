      ...
      <template v-if="isAuthenticated">
        ...
        <aside class="sidebar bg-light p-3 border-end shadow-sm" style="width: 380px; min-width:300px; overflow-y: auto;">
              ...
              <div class="card-body p-2">
                ...
                <div class="btn-group btn-group-sm w-100" role="group">
                  <button @click="confirmDeleteSelectedPoints" class="btn btn-danger" :disabled="isDeletingMultiple">
                    <span v-if="isDeletingMultiple" class="spinner-border spinner-border-sm me-1"></span>
                    <i v-else class="bi bi-trash-fill"></i> <span class="ms-1">Удалить</span>
                  </button>
            ...
            <PointInfoPanel
              ...
              @point-deleted="handlePointsDeleted" 
...
const handleLogout = async () => {
    try {
        if (localStorage.getItem('authToken')) await $axios.post(props.djangoSettings.apiLogoutUrl);
    } catch (error) {
        console.error("Ошибка при выходе на сервере, но клиентская сессия будет очищена:", error);
    } finally {
...
const initializeAppData = async () => {
    ...
    if (userPermissions.canUpload) promises.push(fetchInitialStationNames());
    try {
...
const handlePointUpdated = (updatedFeature) => {
    ...
    if (index !== -1) mapPoints.value[index] = updatedFeature;
    if (userPermissions.canUpload) fetchInitialStationNames();
};

const handlePointsDeleted = (deletedPointIds = []) => { 
    const deletedIdsSet = new Set(deletedPointIds.map(String));
    mapPoints.value = mapPoints.value.filter(p => !deletedIdsSet.has(String(p.properties?.id)));
    clearSelection();
};

const confirmDeleteSelectedPoints = () => {
  if (!selectedPointIds.value.length) return;
  const message = `Вы уверены, что хотите удалить ${selectedPointIds.value.length} выделенных пунктов?\n\nВНИМАНИЕ: Это также безвозвратно удалит ВСЕ связанные с ними файлы наблюдений с сервера.`;
...
const deleteSelectedPoints = async () => {
  ...
  const deleteUrl = `${String(props.djangoSettings.apiPointsUrl).replace(/\/$/, '')}/delete-points/`;
  try {
    ...
    handlePointsDeleted(response.data.deleted_point_ids);
  } catch (error) {
    addUserMessage({ type: 'danger', text: error.response?.data?.detail || error.response?.data?.error || 'Ошибка группового удаления.' });
  } finally {
...
const handleUploadComplete = async (uploadResult) => {
    ...
    if (uploadResult.success || uploadResult.total_created_count > 0) {
        ...
        if (userPermissions.canUpload) await fetchInitialStationNames();
    }
};
...
const handleKMLUpload = async ({ formData }) => {
  ...
  try {
    ...
    if (response.data.success && response.data.updated_count > 0) await fetchMapPoints();
  } catch (error) {
